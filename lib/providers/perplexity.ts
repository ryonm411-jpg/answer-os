...
export function isConfigured(): boolean {
  return Boolean(process.env.PERPLEXITY_API_KEY && process.env.PERPLEXITY_API_KEY.trim().length > 0);
}
...
export class PerplexityProvider implements AIProvider {
  ...
  async ask(
    ...
    try {
      ...
      const modelId = resolveModel("perplexity", config.model);
      ...
      throw toProviderError("perplexity", err);
    }
  }
}
